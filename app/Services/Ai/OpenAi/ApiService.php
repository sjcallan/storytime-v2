...
class ApiService implements AiApiServiceInterface
{
    ...
    public function __construct()
    {
        ...
        $this->baseUrl = config('ai.providers.openai.base_url') ?? config('services.openai.base_url', 'https://api.openai.com/v1');
        $this->model = config('ai.providers.openai.model', 'gpt-4.1');
        $this->maxTokens = config('ai.providers.openai.max_tokens', 4000);
        $this->temperature = config('ai.providers.openai.temperature', 0.8);
    }
