...
class CharacterChatService
{
     ...
     */
    protected function stripQuotes(string $message = null):string|null
    {
        if(!$message) {
            return null;
        }
    ...
    {
        $context = [
            'You are an author writing an ' . strtolower($this->character->book->genre) . ' novel for ' . $this->character->book->age_level . ' year olds. The main character is named ' . $this->character->name . '. Here is their details: ',
        ];

        if($this->character->description) {
            $context[] = 'Description: ' . $this->character->description;
        }

        if($this->character->age) {
            $context[] = 'Age: ' . $this->character->age;
        }

        if($this->character->age) {
            $context[] = 'Gender: ' . $this->character->gender;
        }

        if($this->character->backstory) {
            $context[] = 'Backstory: ' . $this->character->backstory;
        }

        $chapterExperiences = [];
        foreach($this->character->chapters AS $i => $chapter) {
            $message = $this->character->name  . '\'s memory ' . $i . ': ';
            ...
            if($chapter->pivot->inner_thoughts) {
                $message .= '### ' . $this->character->name  . '\'s inner thoughts were ' . $chapter->pivot->inner_thoughts;
            }

            if($chapter->pivot->goals) {
                $message .= '### ' . $this->character->name  . '\'s goal was ' . $chapter->pivot->goals;
            }
        ...
        $context[] = 'Answer questions as ' . $this->character->name . ' would. ### respond with only ' . $this->character->name . '\'s spoken words.### Respond to me by name.';
    ...
    protected function addPersonaContext()
    {
        ...
        $context = [
            ...
            'Answer questions as ' . $this->character->name . ' would.',
            'Respond with only ' . $this->character->name . '\'s spoken words. respond with no less than 800 words.'
        ];
